...
- **Anthropic:** we are building with Claude Code so we are technically eligible, but a debate game is a small swing in a non-target domain. Do not spend design effort chasing this prize. It is a freebie if it comes, not a goal.
...
- Claude API for the per-turn analysis (fallacy detection, scoring, verdict)
- Recommended model: `claude-sonnet-4-6` for the latency/quality balance needed in a near-real-time loop. Use `claude-haiku-4-5` if a particular call needs to be even faster (e.g. quick foul callouts). Output must be structured JSON.
...
4. **Analyze each completed turn:**
   ...
   b. Send the turn text plus the retrieved fallacy context to Claude.
   c. Claude returns JSON: detected fallacies (type, severity, exact quote, short explanation), an argument-strength rating, and a point delta.
...
8. **End debate:** send the full turn history to Claude for a final verdict and winner, then announce it.
...
## 10. Claude analysis prompt (adapt this)
...
6. Build the Claude analysis route: turn in, JSON flags + score out. Test on hand-written turns.
...
If you run low on time, cut in this order: TTS callouts last-resort kept, Redis can fall back to a hardcoded fallacy list passed inline to Claude, live mic mode dropped entirely. Never cut the recorded-clip core loop or the live scoreboard.
